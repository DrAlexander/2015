...
def main(Vig_key, AES_key, image):
    ...
    for byte in message:
        temp.append(byte)
        if len(temp) == 16:
            decrypte_part = decrypt(temp, AES_key)
            decrypte_.extend(decrypte_part)
            del temp[:]

    decrypte_ = decrypte_[:-1]
    count = decrypte_.count('\x00')
	
    [decrypte_.remove('\x00') for i in range(count)]
    
    res = VigenereDecrypt(decrypte_, Vig_key)
    return res
